variable "location" {
  description = "Azure region for all resources in the stage environment"
  type        = string
...
variable "resource_groups" {
  description = "Map of resource groups to create for the stage environment"
  type = map(object({
    name = string
...
variable "storage_accounts" {
  description = "Map of storage accounts to create for the stage environment"
  type = map(object({
    name                     = string
...
variable "network_interfaces" {
  description = "NIC definitions for VM attachments. Subnet relationships are resolved from the network module output by key."
  type = map(object({
    name                = string
    resource_group_name = string
    location            = string
...
variable "application_gateways" {
  description = "Application Gateway for public ingress. VNet and public IP relationships are resolved from module outputs by key."
  type = map(object({
    name                = string
    resource_group_name = string
    location            = string
...
variable "bastions" {
  description = "Azure Bastion hosts for secure SSH/RDP access. VNet/subnet and public IP relationships are resolved from outputs by key."
  type = map(object({
    name                = string
    resource_group_name = string
    location            = string
